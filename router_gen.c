...
//	float sizetests[NUM_SIZETESTS] = {-4.0, -3.0, -2.0, -1.0, 0, 1.0, 2.0, 3.0, 4.0};
	...
	int markpower = 4;
	float toolsize = 0.12; // How much excess does the laser burn.
...
*/
	...
	fprintf(gfile, "(%f; %f; %f; %f; %f; %f; %f; %f;)\n(%f; %f; %f; %f; %f; %f; %f; %d;)\n(%f; %f; %d; %f; %d; %f; %d; %f)\n",
		...
		cellgap, bonushole, toolsize, finger_size_x, finger_size_y, side_bonushole_size, side_bonushole_dist, num_side_front_fingers,
	...
	for(int curx = 0; curx < x; curx++)
	{
		...
		for(int cury = 0; cury < y; cury++)
		{
			...
			float start_x_trimmed = start_x + toolsize;
			float start_y_trimmed = start_y;

			float offset_i = hole/2.0 - toolsize;
			float offset_j = 0.0;
...
//					)/3.0;
			...
			if(bottom)
			{
				fprintf(gfile, "G00 X%.2f Y%.2f\n", start_x_trimmed+toolsize,
					start_y_trimmed);
				CUT_PWR(0.30);
				fprintf(gfile, "G02 X%.2f Y%.2f I%.2f J%.2f\n", start_x_trimmed+toolsize, start_y_trimmed,
					offset_i-toolsize, offset_j);
				UNCUT();
